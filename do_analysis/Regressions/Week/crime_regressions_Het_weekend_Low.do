...
****drop
sort stat_id year 
by stat_id year: egen O_violent_sum= sum( O_violent_count) if year ==2012
by stat_id : egen O_violent_sum1 = max( O_violent_sum)
sum  O_violent_sum if year == 2012, d 
keep if O_violent_sum1 <`r(p50)'  
drop  O_violent_sum O_violent_sum1
...
global fileoutname "Prelim_Reg_weekend_LOW.xml"



global totalTreat S_violentNSEX_week
reg   $dependent $totalTreat
outreg2 using "${outputfile1}\${fileoutname}" , title("CTA Ridership and Crime,Weekend Low Crime Areas")   replace  slow(1000) 
