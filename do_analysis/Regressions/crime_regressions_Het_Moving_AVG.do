...
global fileoutname "Prelim_RegOTHER.xml"

keep $keep_vars $interesting_vars

replace  rides_numb= ln(rides_numb)

ds *_count ,
foreach crime in `r(varlist)'{

sort stat_id datem
gen `crime'_mov_avg = 0
forvalues i=1(1)22 {
local crimename =subinstr("`crime'","_count","",.)
replace `crimename'_mov_avg= `crimename'_mov_avg +`crimename'_count[_n-`i']  if stat_id[_n] == stat_id[_n-`i'] & datem[_n] ==datem[_n-`i'] +`i' 
}
}
...
global set2 "$set1 S_CRIM_SEXUAL_ASSAULT"
global set3 "$set2 O_CRIM_SEXUAL_ASSAULT"
global set4 "$set3 S_property"
global set5 "$set4 S_ASSAULTsimp S_BATTERYsimp"
global set6 "$set5 S_qual_life_count"
